function startClassification(){
    navigator.mediaDevices.getUserMedia('audio:true');
    classifier = ml5.soundClassifier('https://teachablemachine.withgoogle.com/models/awZ02m2Ty/model.json', modelReady);
}

function modelReady(){
   classifier.classify(gotResults);
}
function gotResults(error,results){
    console.log('gotResults');
    if(error){
        console.error("error");
    }
    else{
        console.log(results);
        random_number_r =  Math.floor(Math.random() * 255) + 1;
        random_number_g =  Math.floor(Math.random() * 255) + 1;
        random_number_b =  Math.floor(Math.random() * 255) + 1;
        console.log(random_number_r);
        console.log(random_number_g);
        console.log(random_number_b);
        document.getElementById("result_label").innerHTML = "I can hear - " + results[0].label;
        document.getElementById("confidence_label").innerHTML = "Accuracy - " + (results[0].confidence*100).toFixed(2) +"%";
        document.getElementById("result_label").style.color = "rgb("+random_number_r+", "+random_number_g+", "+random_number_b+")";
        document.getElementById("confidence_label").style.color = "rgb("+random_number_r+", "+random_number_g+", "+random_number_b+")";

        img1 = document.getElementById("aliens_1");
        img2 = document.getElementById("aliens_2");
        img3 = document.getElementById("aliens_3");
        img4 = document.getElementById("aliens_4");
        
        if(results[0].label=="song"){
            img2.src = 'aliens-02.gif';
            img1.src = 'aliens-01.png';
            img3.src = 'aliens-03.png';
            img4.src = 'aliens-04.png';
        }
        else if(results[0].label=="Background Noise"){
            img3.src = 'aliens-03.gif';
            img1.src = 'aliens-01.png';
            img2.src = 'aliens-02.png';
            img4.src = 'aliens-04.png';
        }
        else if(results[0].label=="clap"){
            img1.src = 'aliens-01.gif';
            img2.src = 'aliens-02.png';
            img3.src = 'aliens-03.png';
            img4.src = 'aliens-04.png';
        }
        else{
            img4.src = 'aliens-04.gif';
            img1.src = 'aliens-01.png';
            img3.src = 'aliens-03.png';
            img2.src = 'aliens-02.png';
        }
    }
}
